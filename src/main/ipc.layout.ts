...
ipcMain.on('layout:minimize', () => MainWindow.instance.minimize());
ipcMain.on('layout:maximize', () => MainWindow.instance.maximize());
ipcMain.on('layout:close', () => MainWindow.instance.close());

ipcMain.on('layout:openPlug', (event, plug: string, url: string) => {
  MainWindow.mainView.webContents.loadURL(resolvePlugPath(plug, url));
});

ipcMain.on(
  'layout:resizeWithMain',
